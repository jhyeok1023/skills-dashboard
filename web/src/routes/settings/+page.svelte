	...
	<svelte:boundary>
		{#if config}
			...
			<section class="card stack">
				...
				<p class="tiny muted" data-value>
					자동 인식은 fluent-bit이 파싱해 둔 log_processed 필드와 Gin 기본 access log를 함께
					읽습니다. 아래에 실제 로그 한 줄을 붙여넣어 규칙이 맞는지 확인한 뒤 저장하세요.
				</p>
				...
				<div class="field preset-field">
					...
					<p class="tiny muted" data-value>
						자동 인식에서는 JSON 필드를 먼저 쓰고, 없으면 Gin access log를 파싱합니다. Gin으로
						고정하면 아래 필드 이름은 쓰이지 않고, Gin 로그에 없는 User-Agent 열도 빠집니다.
					</p>
				</div>
