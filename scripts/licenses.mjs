...
function render(list) {
	...
	// 이 문서는 산출물을 받은 제3자에게 딸려간다. 저장소 문서가 아니므로
	// 영어로 쓰고, 빌드가 어떻게 도는지는 적지 않는다.
	const out = [];
	out.push('# Third-Party Notices');
	out.push('');
	out.push('skills-dashboard incorporates the third-party components listed below. The');
	out.push('copyright notices and the license terms under which those components were');
	out.push('received are reproduced verbatim in this file.');
	out.push('');
	out.push('skills-dashboard itself is licensed under the BSD 3-Clause License; see');
	out.push('`LICENSE`. Nothing in this file alters the terms of any component listed here.');
	out.push('');
	out.push('When redistributing a built artifact, include `LICENSE` and this file with it.');
	out.push('The build places both alongside the artifacts it produces.');
	out.push('');
	out.push('Each component is listed against the artifacts it is built into:');
	out.push('');
	out.push('- `go binary` — `bin/skills-dashboard-<platform>`');
	out.push('- `node bundle` — `server/dist/skills-dashboard.mjs`');
	out.push('- `spa` — `internal/web/dist`, embedded in both of the above');
	out.push('');
	out.push('License names are SPDX identifiers. Identical license texts are printed once,');
	out.push('with the components they cover.');
	out.push('');
	out.push('Generated by `mise run licenses`. Do not edit.');
	out.push('');

	// --- 구성요소 표 ---
	out.push('## Components');
	out.push('');
	out.push(`${list.length} components.`);
	out.push('');
	out.push('| Component | Version | License | Artifact |');
	out.push('| --- | --- | --- | --- |');
	for (const c of list) {
		const spdx = c.spdx ?? c.licenses.map(({ text }) => detectSpdx(text)).find(Boolean) ?? '(see text)';
	...
	if (withNotice.length > 0) {
		out.push('## Notices');
		out.push('');
		out.push('Reproduced as required by section 4(d) of the Apache License, Version 2.0.');
		out.push('');
	...
	for (const [i, g] of ordered.entries()) {
		out.push('---');
		out.push('');
		out.push(`### ${i + 1}. ${g.spdx ?? 'See text below'}`);
		out.push('');
		out.push(`Applies to ${g.members.length} component${g.members.length === 1 ? '' : 's'}:`);
		out.push('');
		for (const m of [...g.members].sort((a, b) => a.name.localeCompare(b.name))) {
			const note = m.borrowedFrom
				? ` — distributed without a license file; the text of its declared \`${m.spdx}\` is reproduced here from \`${m.borrowedFrom}\`.`
				: '';
